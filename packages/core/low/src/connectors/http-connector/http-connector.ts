...
import { Connector, TaskErrorMap, TaskConfig, ConnectorRunError, ObjectCompiler } from '../../index';
    ...
    try {
      ...
      if (Array.isArray(input.site?.config?.outputHandlers)) {
        for (const handler of input.site.config.outputHandlers) {
          const task = this.env.getTask(handler);
          await this.runTask(task, input, match.route.config, { context, output });
        }
      }
  ...
  setResponseBody(response: Http.ServerResponse, body: any, gzip: boolean = false) {
    try {
      ...
      if (Buffer.isBuffer(body)) {
        bodyBuffer = body as Buffer<ArrayBuffer>;
...
export interface Cookie {
  value?: string;
  options?: CookieHelper.SerializeOptions;
}
