...
import { Environment, ConnectorContext, TaskConfig } from '../../index';
...
test('should not be able to convert bits to bools with invalid record sets and references', async () => {
  ...
  expect(() => { doer.handleResults(config, null, singleRecordSet); }).toThrow(/Record set does not contain results/);
});
...
test('should not be able to resolve scalar with wrong number of result sets or rows or columns', async () => {
  ...
  expect(() => { doer.handleResults(config, null, results) }).toThrow(/Record set does not contain results/);
  expect(() => { doer.handleResults(config, null, []) }).toThrow(/Record set does not contain results/);
  expect(() => { doer.handleResults(config, null, [[]]) }).toThrow(/Record set does not contain results/);
  ...
  expect(() => { doer.handleResults(config, null, results2Columns) }).toThrow(/Cannot treat results with more or less than 1 column as a scalar results/);
  expect(() => { doer.handleResults(config, null, results2Rows) }).toThrow(/Record set contains more or less than one row/);

  const multipleRecordSets = [[{ str: 'test' }], [{ str: 'test' }]];
  expect(() => { doer.handleResults(config, null, multipleRecordSets) }).toThrow(/Record set contains more or less than one row/);
});
...
test('should not be able to resolve scalar when result not castable', async () => {
  ...
  expect(() => { doer.handleResults(config, null, results) }).toThrow(/Unexpected token/);
});
