...
import { Environment, TaskConfig, ConnectorContext } from '../../index';
...
test('should be able to bulk insert objects', async () => {
  ...
  const accounts = [...Array(2).keys()].map((i: number) => {
    return {
      Name: `Test Account ${i}`,
      Website: 'https://example.com'
    };
  });
  const accountChunks = chunk(accounts, 2);
  ...
  expect(context.data.insertTask[0].length).toBe(2);
  ...
  const accountIdChunks = chunk(accountIds, 2);
  expect(context.data.queryTask1.length).toBeGreaterThanOrEqual(2);
