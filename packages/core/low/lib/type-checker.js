...
class TypeChecker {
    static parseLogic(tree, level = 1) {
        if (!Array.isArray(tree))
            throw new Error('A logic branch must be an array');
        if (tree.length === 0)
            throw new Error('Passed an empty logic branch');
        if (tree[0] !== 'AND' && tree[0] !== 'OR')
            throw new Error('A logic branch must start with either a "AND" or "OR"');
        if (tree.length === 1)
            throw new Error('A logic branch must contain at least one item');
        let statements = [];
        ...
        for (const item of tree) {
            ...
            if (Array.isArray(item)) {
                ...
                if (typeof item.path !== 'string')
                    throw new Error('Item does not contain a "path" property');
    ...
    static compileLogic(tree) {
        const parsed = TypeChecker.parseLogic(tree);
        return new Function('context', `try { return ${parsed}; } catch(err) { return false; }`);
    }
}
