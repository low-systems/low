...
class HttpConnector extends index_1.Connector {
    constructor() {
        super(...arguments);
        this.sites = {};
        this.hostnameCache = {};
    ...
    requestHandler(request, response) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                const output = yield index_1.ObjectCompiler.compile(match.route.config.output, context);
                if (Array.isArray((_d = (_c = input.site) === null || _c === void 0 ? void 0 : _c.config) === null || _d === void 0 ? void 0 : _d.outputHandlers)) {
                    for (const handler of input.site.config.outputHandlers) {
                        const task = this.env.getTask(handler);
                        yield this.runTask(task, input, match.route.config, { context, output });
                    }
                }
    ...
    handleError(response, error, input) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                let data = {};
                let errors = { error };
                if (error instanceof index_1.ConnectorRunError) {
                    data = error.context.data;
                    errors = error.context.errors;
                }
                const context = yield this.runTask(task, input, config, data, errors);
                const output = yield index_1.ObjectCompiler.compile(handler.output, context);
    ...
    setResponseBody(response, body, gzip = false) {
        try {
            ...
            if (Buffer.isBuffer(body)) {
                ...
                const bodyJson = (0, fast_safe_stringify_1.default)(body);
