      ...
      if (config.interperateStrings && typeof input === 'string') {
        ...
        if (Array.isArray(input)) {
          if (config.removeObjectNullValues) {
            return input.filter(item => !['null', 'undefined'].includes(typeof item)).length > 0;
          } else {
            return input.length > 0;
          }
        } else {
          if (config.removeObjectNullValues) {
            return Object.values(input).filter(value => !['null', 'undefined'].includes(typeof value)).length > 0;
          } else {
...
export const TrueStrings: string[] = [
